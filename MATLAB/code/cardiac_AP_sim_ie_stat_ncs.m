...
sim_result_dir = [storage_dir 'ie_stat_nonconservative_stim_result' filesep];
...
if run_simulation
    ...
    for i_segment = 1:num_segments
        ...
        if i_segment == 1
            [VOI, STATES] = cardiac_AP_exported_ie_stat(tspan,'ncs');
        else
            [VOI, STATES] = cardiac_AP_exported_ie_stat(tspan,'ncs',X0);
        end
        toc
